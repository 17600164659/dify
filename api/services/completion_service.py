...
class CompletionService:
    ...
    def completion(
        ...
        # is streaming mode
        logging.info(str(args["decision_making"]))
        ...
        if conversation_id:
            ...
            conversation = (
                ...
                .filter(and_(*conversation_filter))
                .first()
            )
            ...
            if not conversation.override_model_configs:
                app_model_config = db.session.query(AppModelConfig).get(
                    conversation.app_model_config_id
                )
                ...
                app_model_config = AppModelConfig(
                    ...
                    model_id="",
                    configs="",
                    opening_statement=conversation_override_model_configs[
                        "opening_statement"
                    ],
                    suggested_questions=json.dumps(
                        conversation_override_model_configs["suggested_questions"]
                    ),
                    model=json.dumps(conversation_override_model_configs["model"]),
                    user_input_form=json.dumps(
                        conversation_override_model_configs["user_input_form"]
                    ),
                    pre_prompt=conversation_override_model_configs["pre_prompt"],
                    agent_mode=json.dumps(
                        conversation_override_model_configs["agent_mode"]
                    ),
                )
            ...
            if is_model_config_override:
                ...
                model_config = AppModelConfigService.validate_configuration(
                    account=user, config=args["model_config"], mode=app_model.mode
                )
        ...
        generate_worker_thread = threading.Thread(
            target=cls.generate_worker,
            kwargs={
    ...
    def generate_worker(
        ...
        with flask_app.app_context():
            try:
                ...
                db.session.rollback()
                PubHandler.pub_error(
                    user,
                    generate_task_id,
                    LLMAuthorizationError("Incorrect API key provided"),
                )
    ...
    def countdown_and_close(
        cls, worker_thread, pubsub, user, generate_task_id
    ) -> threading.Thread:
        # wait for 5 minutes to close the thread
        timeout = 300
    ...
    def generate_more_like_this(
        ...
        generate_worker_thread = threading.Thread(
            target=cls.generate_more_like_this_worker,
            kwargs={
                "flask_app": current_app._get_current_object(),
                "generate_task_id": generate_task_id,
                "app_model": app_model,
    ...
    @classmethod
    def generate_more_like_this_worker(
        cls,
        flask_app: Flask,
        generate_task_id: str,
        app_model: App,
        ...
        for config in input_form_config:
            ...
            if variable not in user_inputs or not user_inputs[variable]:
                if "required" in input_config and input_config["required"]:
                    ...
                    filtered_inputs[variable] = (
                        input_config["default"] if "default" in input_config else ""
                    )
                    continue
            ...
            if input_type == "select":
                ...
                if value not in options:
                    raise ValueError(
                        f"{variable} in input form must be one of the following: {options}"
                    )
            else:
                if "max_length" in variable:
                    ...
                    if len(value) > max_length:
                        raise ValueError(
                            f"{variable} in input form must be less than {max_length} characters"
                        )
    ...
    def compact_response(
        cls, pubsub: PubSub, streaming: bool = False
    ) -> Union[dict | Generator]:
                        ...
                        if message["type"] == "message":
                            ...
                            if event == "message":
                                yield "data: " + json.dumps(
                                    cls.get_message_response_data(result.get("data"))
                                ) + "\n\n"
                            elif event == "chain":
                                yield "data: " + json.dumps(
                                    cls.get_chain_response_data(result.get("data"))
                                ) + "\n\n"
                            elif event == "agent_thought":
                                yield "data: " + json.dumps(
                                    cls.get_agent_thought_response_data(
                                        result.get("data")
                                    )
                                ) + "\n\n"
                except ValueError as e:
                    if (
                        e.args[0] != "I/O operation on closed file."
                    ):  # ignore this error
                        logging.exception(e)
                        raise
                finally:
