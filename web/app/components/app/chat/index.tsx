...
const MoreInfo: FC<{ more: MessageMore; isQuestion: boolean }> = ({
  more,
  isQuestion,
}) => {
  ...
  return (
    <div
      className={`mt-1 space-x-2 text-xs text-gray-400 ${isQuestion ? "mr-2 text-right " : "ml-2 text-left float-right"
        }`}
    >
      <span>{`${t("appLog.detail.timeConsuming")} ${more.latency}${t(
        "appLog.detail.second"
      )}`}</span>
      <span>{`${t("appLog.detail.tokenCost")} ${formatNumber(
        more.tokens
      )}`}</span>
      <span>· </span>
      <span>{more.time} </span>
...
const TryToAskIcon = (
  <img
    width={11}
    height={10}
    src="https://assets.metaio.cc/assets/difyassets/szww.png"
  />
)
        ...
        content={
          (isWebScene || (!isUserFeedback && !isWebScene)
            ...
            : !isWebScene && isUserFeedback
              ? `${t("appDebug.operation.userAction")}${isLike
    ...
    <div key={id}>
      <div className="flex items-start">
        <img
          src={
            appIcon ||
            "https://assets.metaio.cc/assets/difyassets/bisailogo.png"
          }
...
type IQuestionProps = Pick<
  IChatItem,
  "id" | "content" | "more" | "useCurrentUserAvatar"
>
        ...
        >
          ...
          <div className="relative answer-textarea custom-no-border">
            <AutoHeightTextarea
              ...
              className={`${cn(
                s.textArea
              )} resize-none block w-full pl-3 rounded-md  focus:outline-none sm:text-sm text-gray-700 custom-no-border`}
            />
            <div className="absolute top-0 right-2 flex items-center h-[48px]">
              ...
              {query ? (
                ...
                >
                  ...
                  <Microphone01Solid className="hidden w-4 h-4 text-gray-700 group-hover:block" />
                </div>
              ) : null}
