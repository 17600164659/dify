    ...
    def model_config(self):
        ...
        if self.override_model_configs:
            ...
            if "model" in override_model_configs:
                ...
                model_config["opening_statement"] = override_model_configs[
                    "opening_statement"
                ]
                model_config["suggested_questions"] = override_model_configs[
                    "suggested_questions"
                ]
                model_config["suggested_questions_after_answer"] = (
                    override_model_configs["suggested_questions_after_answer"]
                    if "suggested_questions_after_answer" in override_model_configs
                    else {"enabled": False}
                )
                model_config["speech_to_text"] = (
                    override_model_configs["speech_to_text"]
                    if "speech_to_text" in override_model_configs
                    else {"enabled": False}
                )
                model_config["more_like_this"] = (
                    override_model_configs["more_like_this"]
                    if "more_like_this" in override_model_configs
                    else {"enabled": False}
                )
                model_config["user_input_form"] = override_model_configs[
                    "user_input_form"
                ]
            else:
...
class Message(db.Model):
    ...
    # decision_making = db.Column(db.Boolean, nullable=False, server_default=False)
...
class Site(db.Model):
    ...
    def app_base_url(self):
        return (
            current_app.config["APP_WEB_URL"]
            if current_app.config["APP_WEB_URL"]
            else request.host_url.rstrip("/")
        )
