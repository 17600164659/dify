...
// )
const TryToAskIcon = (
  <img width={11} height={10} src="https://assets.metaio.cc/assets/difyassets/szww.png" />
)
...
const Answer: FC<IAnswerProps> = ({ item, feedbackDisabled = false, isHideFeedbackEdit = false, onFeedback, onSubmitAnnotation, displayScene = 'web', isResponsing, answerIconClassName, isLast }) => {
  ...
  return (
    <div key={id}>
      <div className='flex items-start'>
        <img src={appIcon || "https://assets.metaio.cc/assets/difyassets/cxctlogo.png"} className='w-10 h-10 shrink-0' style={{ borderRad: 10 }} />
...
const Chat: FC<IChatProps> = ({
  ...
  return (
    <div className={cn('px-3.5', 'h-full')}>
      {/* Chat List */}
      <div className="h-full space-y-[30px]" style={{ paddingTop: 20 }}>
        {chatList.map((item) => {
          if (item.isAnswer) {
          ...
          return <Question isAnswer={item.isAnswer} key={item.id} id={item.id} content={item.content} more={item.more} useCurrentUserAvatar={useCurrentUserAvatar} />
        })}
      </div>
      <img src="" />
      {
