...
class Config:
    ...
    def __init__(self):
        # app settings
        self.CONSOLE_API_URL = (
            get_env("CONSOLE_URL")
            if get_env("CONSOLE_URL")
            else get_env("CONSOLE_API_URL")
        )
        self.CONSOLE_WEB_URL = (
            get_env("CONSOLE_URL")
            if get_env("CONSOLE_URL")
            else get_env("CONSOLE_WEB_URL")
        )
        self.SERVICE_API_URL = (
            get_env("API_URL") if get_env("API_URL") else get_env("SERVICE_API_URL")
        )
        self.APP_WEB_URL = (
            get_env("APP_URL") if get_env("APP_URL") else get_env("APP_WEB_URL")
        )
        self.APP_API_URL = (
            get_env("APP_URL") if get_env("APP_URL") else get_env("APP_API_URL")
        )
