...
class ConversationMessageTask:
    ...
    def init(self):
        provider_name = LLMBuilder.get_default_provider(
            self.app.tenant_id, self.model_name
        )
        ...
        if self.mode == "chat":
            ...
            if self.app_model_config.pre_prompt:
                system_message = PromptBuilder.to_system_message(
                    self.app_model_config.pre_prompt, self.inputs
                )
                system_instruction = system_message.content
                llm = LLMBuilder.to_llm(self.tenant_id, self.model_name)
                system_instruction_tokens = llm.get_num_tokens_from_messages(
                    [system_message]
                )
        ...
        if not self.conversation:
            ...
            self.conversation = Conversation(
                ...
                from_end_user_id=(
                    self.user.id if isinstance(self.user, EndUser) else None
                ),
                from_account_id=(
                    self.user.id if isinstance(self.user, Account) else None
                ),
            )
    ...
    def save_message(self, llm_message: LLMMessage, by_stopped: bool = False):
        ...
        total_price = self.calc_total_price(
            message_tokens, message_unit_price, answer_tokens, answer_unit_price
        )
        ...
        self.message.answer = (
            PromptBuilder.process_template(llm_message.completion.strip())
            if llm_message.completion
            else ""
        )
    ...
    def on_agent_end(
        self, message_chain: MessageChain, agent_model_name: str, agent_loop: AgentLoop
    ):
        agent_message_unit_price = llm_constant.model_prices[agent_model_name]["prompt"]
        agent_answer_unit_price = llm_constant.model_prices[agent_model_name][
            "completion"
        ]
        ...
        message_agent_loop = MessageAgentThought(
            ...
            currency=llm_constant.model_currency,
            created_by_role=(
                "account" if isinstance(self.user, Account) else "end_user"
            ),
    ...
    def calc_total_price(
        self, message_tokens, message_unit_price, answer_tokens, answer_unit_price
    ):
        message_tokens_per_1k = (decimal.Decimal(message_tokens) / 1000).quantize(
            decimal.Decimal("0.001"), rounding=decimal.ROUND_HALF_UP
        )
        answer_tokens_per_1k = (decimal.Decimal(answer_tokens) / 1000).quantize(
            decimal.Decimal("0.001"), rounding=decimal.ROUND_HALF_UP
        )

        total_price = (
            message_tokens_per_1k * message_unit_price
            + answer_tokens_per_1k * answer_unit_price
        )
        return total_price.quantize(
...
class PubHandler:
    def __init__(
        ...
        chain_pub: bool = False,
        agent_thought_pub: bool = False,
    ):
    ...
    def pub_error(cls, user: Union[Account | EndUser], task_id: str, e):
        content = {
            ...
            if getattr(e, "description", None) is not None
            else str(e),
        }
