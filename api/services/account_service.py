...
class TenantService:
    ...
    def check_member_permission(
        ...
        perms = {"add": ["owner", "admin"], "remove": ["owner"], "update": ["owner"]}
...
class RegisterService:
    ...
    def invite_new_member(
        cls, tenant: Tenant, email: str, role: str = "normal", inviter: Account = None
    ) -> str:
        """Invite new member"""
        account = Account.query.filter_by(email=email).first()
    ...
    @classmethod
    def get_account_if_token_valid(
        cls, workspace_id: str, email: str, token: str
    ) -> Optional[Account]:
        tenant = (
            db.session.query(Tenant)
            .filter(Tenant.id == workspace_id, Tenant.status == "normal")
            .first()
        )
        ...
        tenant_account = (
            db.session.query(Account, TenantAccountJoin.role)
            .join(TenantAccountJoin, Account.id == TenantAccountJoin.account_id)
            .filter(Account.email == email, TenantAccountJoin.tenant_id == tenant.id)
            .first()
        )
    ...
    @classmethod
    def _get_account_id_by_invite_token(
        cls, workspace_id: str, email: str, token: str
    ) -> Optional[str]:
        email_hash = sha256(email.encode()).hexdigest()
        cache_key = "member_invite_token:{}, {}:{}".format(
