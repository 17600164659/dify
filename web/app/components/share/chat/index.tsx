...
import { decision, execute, pluginCall } from './decision';
...
const Main: FC<IMainProps> = ({ isInstalledApp = false, installedAppInfo }) => {
  ...
  useEffect(() => {
    if (siteInfo?.title) {
      ...
      else document.title = `${siteInfo.title} - iPollo.AI`
    }
  }, [siteInfo?.title, plan])
  ...
  const [isShare, { setTrue: setShareTrue, setFalse: setShareFalse }] =
    useBoolean(false)
  const [isBeginNew, { setTrue: setBeginNewTrue, setFalse: setBeginNewFalse }] =
    useBoolean(false)
  ...
  const conversationName =
    currConversationInfo?.name || (t("share.chat.newChatDefaultName") as string)
  ...
  useEffect(() => {
    (async () => {
      ...
      if ((is_new !== undefined && is_new !== null) || preConversation) {
      ...
      try {
        ...
        if (isNotNewConversation && !is_new)
          setCurrConversationId(
            preConversation || _conversationId,
            appId,
            false
          )
  ...
  const handleSend = async (message: string) => {
    ...
    try {
      if (strategy && strategy.length) {
        const pluginValue = await pluginCall({ ...data }, strategy, appData)
        if (pluginValue) {
        ...
        {isMobile && isShowSidebar && (
          ...
          >
            <div
              ...
              style={{ display: isShare ? "inline-block" : "inline-flex" }}
