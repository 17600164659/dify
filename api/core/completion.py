...
from core.conversation_message_task import (
...
class Completion:
    ...
    def generate(
        ...
        main_chain = MainChainBuilder.to_langchain_components(
            ...
            memory=ReadOnlyConversationTokenDBStringBufferSharedMemory(memory=memory)
            if memory
            else None,
    ...
    def run_final_llm(
        ...
        final_llm = LLMBuilder.to_llm_from_model(
            tenant_id=tenant_id, model=app_model_config.model_dict, streaming=streaming
        )
...
"""
            ...
            prompt_content = prompt_template.format(query=query, **prompt_inputs)
            ...
            if pre_prompt:
                pre_prompt_inputs = {
                    k: inputs[k]
                    for k in JinjaPromptTemplate.from_template(
                        template=pre_prompt
                    ).input_variables
                    if k in inputs
                }
...
"""
            ...
            if memory:
                ...
                curr_message_tokens = memory.llm.get_num_tokens_from_messages(
                    [tmp_human_message]
                )
            ...
            human_message = PromptBuilder.to_human_message(
                prompt_content=human_message_prompt, inputs=human_inputs
            )
    ...
    @classmethod
    def get_llm_callbacks(
        cls,
        llm: Union[StreamableOpenAI, StreamableChatOpenAI],
        streaming: bool,
        conversation_message_task: ConversationMessageTask,
    ...
    @classmethod
    def get_history_messages_from_memory(
        cls, memory: ReadOnlyConversationTokenDBBufferSharedMemory, max_token_limit: int
    ) -> str:
        """Get memory messages."""
    ...
    @classmethod
    def get_memory_from_conversation(
        cls,
        tenant_id: str,
        app_model_config: AppModelConfig,
        conversation: Conversation,
        **kwargs,
        ...
        memory_llm = LLMBuilder.to_llm_from_model(
            tenant_id=tenant_id, model=app_model_config.model_dict
        )
    ...
    def get_validate_rest_tokens(
        ...
        max_tokens = app_model_config.model_dict.get("completion_params").get(
            "max_tokens"
        )
    ...
    @classmethod
    def recale_llm_max_tokens(
        cls,
        final_llm: BaseLanguageModel,
        model: dict,
    ...
    def generate_more_like_this(
        ...
        prompt = prompt.format(
            prompt=original_prompt, original_completion=original_completion
        )
